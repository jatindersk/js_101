...
//  iterate through the string and look for (?, ! or .) characters
//  if found a matching character :
//  - slice the string upto the matching character
//  - push the sliced substring into an array
...
let longestSentence = str => {
  ...
  for (let idx = 0; idx < wordsArr.length; idx += 1) {
    ...
    if ((`.!?`.includes(word[word.length - 1]))) {
      let endIdx = idx + 1;
      sentences.push(wordsArr.slice(startIdx, endIdx));
      startIdx = idx + 1;
    }
  }

  let longestSentence = findLongest(sentences);

  console.log(`${longestSentence.join(' ')}`);
  console.log(`The longest sentence has ${longestSentence.length} words.`);
};

function findLongest(sentences) {
